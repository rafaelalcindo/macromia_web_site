...
if($servico != " " && $estado != " " && $cidade != " " && $id_foto != null){
	...
	//	echo "<h3>No momento estamos sem eventos</h3>";
		...
		while($row = mysqli_fetch_array($lista_cli)){
			$nome = $row['nome'];  $email   = $row['email'];   $telefone = $row['Telefone'];  $celular = $row['Celular'];  $codigo	 = $row['codigo']; $complementar = $row['complementar'];
			...
			if($vagas > 0){
				    $id_list += 1;
					$corpo = preencheLista( $nome,$email, $telefone,$celular, $codigo, $data,$hora,$servico, $vagas,$numero,$cep,$endereco,$val_min,$val_max,$estado,$cidade,$id_foto,$credi_foto,$id_list, $pg, $complementar);
				@header('Content-type: text/html; charset=ISO-8859-1');
				echo $corpo;
			}
		}
	...
	//	echo "<h3>No momento estamos sem eventos</h3>";
		...
		while($row = mysqli_fetch_array($lista_cli02)){
			...
			if($vagas > 0){
				$id_list += 1;
				$corpo2 = preencheLista($nome,$email,$telefone,$celular, $codigo, $data,$hora,$servico, $vagas,$numero,$cep,$endereco,$val_min,$val_max,$estado,$cidade,$id_foto,$credi_foto, $id_list, $pg,$complementar);			
				echo $corpo2;
			}
		}
	...
	//	echo "<h3>No momento estamos sem eventos</h3>";
		...
		while($row = mysqli_fetch_array($lista_cli04)){
			...
			if($vagas > 0){
				$id_list += 1;
				$corpo4 = preencheLista($nome,$email,$telefone,$celular, $codigo, $data,$hora,$servico, $vagas,$numero,$cep,$endereco,$val_min,$val_max,$estado,$cidade,$id_foto,$credi_foto, $id_list, $pg, $complementar);
				echo $corpo4;
			}
		}
	...
	//	echo "<h3>No momento estamos sem eventos</h3>";
		...
		while($row = mysqli_fetch_array($lista_cli05)){
			...
			if($vagas > 0){
				$id_list += 1;
				$corpo5 = preencheLista($nome,$email,$telefone,$celular, $codigo, $data,$hora,$servico, $vagas,$numero,$cep,$endereco,$val_min,$val_max,$estado,$cidade,$id_foto,$credi_foto, $id_list, $pg, $complementar);
				echo $corpo5;
			}
		}
	...
	while($row = mysqli_fetch_array($lista_cli03)){
		...
		if($vagas > 0){
			$id_list += 1;
			$corpo3 = preencheLista($nome,$email,$telefone,$celular, $codigo, $data,$hora,$servico, $vagas,$numero,$cep,$endereco,$val_min,$val_max,$estado,$cidade,$id_foto,$credi_foto, $id_list, $pg, $complementar);
			echo $corpo3;
		}
	}
	...
	if($lista_cli06 == false){
	   ...
	   if($lista_cli_06_count > 1 && $pg <= $lista_cli_06_count){
	   	  echo "<div class='clearfix colelem shared_content' id='u364008' style='margin-left: 50%;' data-content-guid='u364008_content'>";
	   		if($pg > 1){
	   			echo "
	   				<div class='rounded-corners clearfix grpelem' id='u50015-4' style='cursor: pointer;' onclick='paginacao_pedidos($pg - 1)' data-muse-uid='U50015' data-muse-type='txt_frame'><!-- content -->
			         <p id='u50015-2'><span id='u50015'>ANTERIOR</span></p>
			        </div>";
	   		}else{
	   			echo"
	   				<div class='rounded-corners clearfix grpelem' id='u367152-4' data-muse-uid='U367152' data-muse-type='txt_frame'><!-- content -->
			         <p id='u367152-2'><span id='u367152'>ANTERIOR</span></p>
			        </div>";
	   		}
	   		...
	   		if($pg < $lista_cli_06_count){
	   			...
	   			echo "
	   				<div class='rounded-corners clearfix grpelem' id='u367151-4' style='cursor: pointer;' onclick='paginacao_pedidos($pg + 1)' data-muse-uid='U367151' data-muse-type='txt_frame'><!-- content -->
			          <p id='u367151-2'><span id='u367151'>PROXIMO</span></p>
			        </div>
	   			";
	   			
	   		}else{
	   			echo "
	   				<div class='rounded-corners clearfix grpelem' id='u367151-4' data-muse-uid='U367151' data-muse-type='txt_frame'><!-- content -->
			          <p id='u367151-2'><span id='u367151'>PROXIMO</span></p>
			        </div>";
	   		}
	   	   echo "</div>";
