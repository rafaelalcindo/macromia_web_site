<?php
require('cadaPedido03.php');
function preencheLista($nome, $email, $telefone, $celular, $codigo,$data, $hora, $servico, $vagas, $numero, $cep, $endereco, $val_min, $val_max, $estado, $cidade, $id_foto, $credi_foto, $id_lista, $pg, $complementar){
	...
	if($verEventFoto == true){	
	 /*	echo "
		...
		"; */
		$listagemPedidos = PedidosComprado($nome, $email,$telefone,$celular,$codigo,$data,$hora,$servico,$vagas,$numero,$cep,$endereco,$val_min,$val_max,$estado,$cidade,$id_foto,$id_lista,$pg,$creditos, $complementar);
		...
		$listagemPedidos02 = MostrarPedidos($estado,$cidade,$id_foto,$data,$codigo,$hora,$val_min,$val_max,$creditos,$credi_foto,$vagas,$pg,$complementar);
		...
		if($orc_min > 0 && $orc_max > 0){
			if($ordenar == 'DESC'){
				$sql = sprintf("select 
					...
					ev.complementar as 'complementar',
					ev.cod_evento as 'codigo', ev.data as 'data', ev.hora as 'hora', ev.servico as 'Servico', ev.vagas as 'vagas',
					le.loca_numero_ev 'numero', le.loca_cep_ev as 'cep', le.loca_end_ev as 'endereco',
					orc.orc_valor_min as 'val_min', orc.orc_valor_max as 'val_max',
					es.uf as 'estado', cid.nome as 'cidade', es.id as 'id_estado', cid.id as 'id_nome'
					
					from cliente cli, evento ev, localizacao_evento le, orcamento orc, estado es, cidade cid
					where cli.cli_cod = ev.cli_cod and ev.cod_evento = le.cod_evento 
		...
		if($ordenar == 'DESC'){
			...
			$sql = sprintf("select 
				cli.cli_nome as 'nome', cli.cli_email as 'email', cli.cli_telefone as 'Telefone', cli.cli_cel as 'Celular',
					...
					and orc.cod_evento = ev.cod_evento
					and le.loca_estado_ev = es.id
					and cid.id =  le.local_cidade_ev ORDER BY ev.cod_evento ASC limit %u, %u",$start,$limite);
		}
