<?php
	...
	// Cadastro do GCM do Fotografo ------------------------------------------------------------------------------------


	function insert_GCM($conexao,$email,$gcm_code){

		$id_foto = getFotoFisiIdByEmail($conexao, $email);
		$insert_GCM = sprintf("update fotografo set foto_gcm = '%s' where foto_cod = '%s' ",$gcm_code, $id_foto);

		$resultado = mysqli_query($conexao, $insert_GCM) or die(mysql_error($conexao)."<br/>".$insert_GCM);
